...
object Core {
    ...
    fun init() {
        if (!inited) {
            ...
            //清理temp文件夹
            WorkDir.globalServiceConfig.tempDir.deleteRecursively()
            WorkDir.globalServiceConfig.tempDir.mkdirs()
            inited = true
        }
    }
    object Inc {
...
//    val policy2 = SizeBasedTriggeringPolicy.createPolicy("1MB")
    ...
    val fileAppender = FileAppender.newBuilder()
        ...
        .setImmediateFlush(true)
        .withFileName(File(WorkDir.globalServiceConfig.logsDir, "$currentTime.log").absolutePath)
...
//    "true", refs, null, serviceConfig, null)
    loggerConfig1.addAppender(appender, Level.INFO, null)
